...
public:
  ...
  inline void ntt(int, int, const int);
  inline int2048 mul(int2048, int2048, const int);
  inline int2048 half();
  inline int2048 more();
  inline friend int2048 lmove(int2048, int);
  inline friend int2048 rmove(int2048, int);
  inline friend int2048 div(int2048, int2048);
  inline int length();
  inline int2048 newton_inv(int);
  inline friend int2048 div(int2048, int);
  // 构造函数
  inline int2048();
  inline int2048(bool);
  inline int2048(int);
  inline int2048(long long);
  inline int2048(const std::string &);
  ...
  // 读入一个大整数
  inline void read(const std::string &);
  // 输出储存的大整数，无需换行
  inline void print();

  // 加上一个大整数
  inline int2048 &add(const int2048 &);
  // 返回两个大整数之和
  inline friend int2048 add(int2048, const int2048 &);

  // 减去一个大整数
  inline int2048 &minus(const int2048 &);
  // 返回两个大整数之差
  inline friend int2048 minus(int2048, const int2048 &);

  // ===================================
  // Integer2
  // ===================================

  inline int2048 operator+() const;
  inline int2048 operator-() const;
...
inline i128 qpow(i128 a, i128 b, i128 mod){
    i128 ans = 1;
    a %= mod;
    while(b){
...
inline i128 smul(i128 a, i128 b, i128 mod){
    i128 ans = 0;
    while(b){
        if(b&1)ans = (ans + a) % mod;
        a = (a + a) % mod;
...
int rev[Maxn];
inline void rev_init(int n, int cou){
    for(int i=0;i<n;i++){
...
inline void g_init(int lgN, const int mod){
    pt = mem;
    //std::cerr << lgN << '\n';
    for(int i=1;i<=lgN;i++){
...
inline void int2048::sizen(int x){
    pol.resize(x);
    return;
}
inline int& int2048::operator[](int x){ return pol.at(x); }
...
inline void int2048::ntt(int n, int type, const int mod){
    for(int i=0;i<n;i++){
...
inline int2048 int2048::mul(int2048 now, int2048 oth, const int mod){
...
inline int2048::int2048(long long y){
    i128 x = y;
    neg = false;
    if(x < 0){
        neg = true;
...
inline int2048::int2048(const int2048 &x){
    pol = x.pol, neg = x.neg;
    //assert(pol.back() || pol.size() == 1);
...
inline double int2048::toDouble(){
...
inline std::string int2048::toString(){
    std::string ret;
    int2048 now = *this;
...
inline bool operator==(const int2048 &inow, const int2048 &ioth){
...
inline bool operator!=(const int2048 &now, const int2048 &oth){
    assert(now.pol.back() || now.pol.size() == 1);
...
inline int2048 add(int2048 now, const int2048 &ioth){
    if(now.neg && now.pol.size() == 1 && now[0] == 0)now.neg = false;
    int2048 oth = ioth;
    if(oth.neg && oth.pol.size() == 1 && oth[0] == 0)oth.neg = false;
    assert(now.pol.back() || now.pol.size() == 1);
...
inline int2048& int2048::add(const int2048 &oth){
    return *this = sjtu::add(*this, oth);
}
inline int2048 minus(int2048 now, const int2048 &ioth){
    return add(now, -ioth);
}
inline int2048& int2048::minus(const int2048 &oth){
    return *this = sjtu::minus(*this, oth);
}
inline int2048& int2048::operator=(const int2048 &oth){
    if(this == &oth){
        return *this;
    }
...
inline int2048 operator+(int2048 now, const int2048 &oth){
    return add(now, oth);
}
inline int2048& int2048::operator+=(const int2048 &oth){
    add(oth);
...
inline int2048 lmove(int2048 now, int k){
    now.pol.insert(now.pol.begin(), k, 0);
    return now;
}
inline int2048 rmove(int2048 now, int k){
    int2048 ret = 0;
    ret.neg = now.neg;
    if(k >= now.pol.size()){
        ret.neg = false;
...
inline int2048 div(int2048 now, int k){
...
inline int2048 div(int2048 now, int2048 oth){
...
inline int2048 int2048::newton_inv(int n){
...
inline int2048 operator/(int2048 now, const int2048 &ioth){
    //assert(false);
    if(now.neg && now.pol.size() == 1 && now[0] == 0)now.neg = false;
    int2048 oth = ioth;
...
inline std::istream &operator>>(std::istream &is, int2048 &now){
    std::string str;
    is>>str;
    now.read(str);
    return is;
}
inline std::ostream &operator<<(std::ostream &os, const int2048 &inow){
    int2048 now = inow;
    if(now.neg && now.pol.size() == 1 && now[0] == 0){
        now.neg = false;
        assert(false);
